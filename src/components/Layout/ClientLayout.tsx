...
function ClientLayout({ children, isAdmin }: Props) {
  const router = useRouter();
  const pathname = usePathname();
  const validatePathnameIfEmployeePage = pathname.split("/")[1] === "employee";
          ...
          <ul className=" w-full px-5 space-y-2">
            {
              <>
                {isAdmin && (
                  <>
                    {SideMenuLinksAdmin.map((item, index) => (
                      <Menu
                        key={index}
                        itemMenu={item}
                        isTab={isTab}
                ...
                {!isAdmin && (
                  <>
                    {SideMenuLinksEmployee.map((item, index) => (
                      <Menu
                        key={index}
                        itemMenu={item}
                        isTab={isTab}
